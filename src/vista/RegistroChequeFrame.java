...
public class RegistroChequeFrame extends javax.swing.JFrame {
    ...
    public void filtrarTabla(String texto) {
        DefaultTableModel modeloTab = (DefaultTableModel) tblMostrar.getModel();
        modeloTab.setRowCount(0);

        ChequeDAO dao = new ChequeDAO();
        List<Cheque> lista = dao.buscarCheques(texto);
    ...
    public void listar() {
        ...
        modeloTab.setRowCount(0);
        ChequeDAO dao = new ChequeDAO();
        List<Cheque> lista = dao.listarCheques();
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        if (cmbCuentaBancaria.getSelectedIndex() <= 0) {
            JOptionPane.showMessageDialog(this, "Seleccione una cuenta bancaria.");
            return;
        }
        ...
        try {
            ...
            String itemSeleccionado = cmbCuentaBancaria.getSelectedItem().toString();

            String nombreBanco = itemSeleccionado.split("-")[0].trim();

            int idEncontrado = 0;
            modelo.Conexion con = new modelo.Conexion();
            java.sql.Connection cn = con.getConexion();

            java.sql.PreparedStatement ps = cn.prepareStatement("SELECT id_cuenta FROM cuentas_bancarias WHERE banco = ? LIMIT 1");
            ps.setString(1, nombreBanco);
            java.sql.ResultSet rs = ps.executeQuery();

            if (rs.next()) {
                idEncontrado = rs.getInt("id_cuenta");
            }

            if (idEncontrado == 0) {
                JOptionPane.showMessageDialog(this, "Error: No se encontró la cuenta '" + nombreBanco + "' en la base de datos.");
                return;
            }

            cheque.setIdCuenta(idEncontrado);
            cheque.setNumeroCheque(txtNumeroCheque.getText());
            cheque.setBeneficiario(txtBeneficiario.getText());
            cheque.setMonto(Double.parseDouble(txtMonto.getText()));
            cheque.setConcepto(txtConcepto.getText());

            try {
                java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("dd/MM/yyyy");
                java.util.Date f = sdf.parse(ftxtFecha.getText());
                cheque.setFecha(new java.sql.Date(f.getTime()));
            } catch (Exception e) {
                cheque.setFecha(new java.sql.Date(System.currentTimeMillis()));
            }

            modelo.ChequeDAO dao = new modelo.ChequeDAO();
            if (dao.registrarCheque(cheque)) {
                JOptionPane.showMessageDialog(this, "Cheque registrado y movimiento de salida generado.");
                limpiarCampos();
                listar();
            } else {
    ...
    private void tblMostrarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblMostrarMouseClicked
        ...
        idSeleccionado = Integer.parseInt(tblMostrar.getValueAt(fila, 0).toString());
        String numCheque = tblMostrar.getValueAt(fila, 1).toString();
        String beneficiario = tblMostrar.getValueAt(fila, 2).toString();
        String monto = tblMostrar.getValueAt(fila, 3).toString();
        String fechaSQL = tblMostrar.getValueAt(fila, 4).toString(); 
        String concepto = tblMostrar.getValueAt(fila, 5).toString();
        ...
        try {
            ...
            java.util.Date date = formatoEntrada.parse(fechaSQL);
            ftxtFecha.setText(formatoSalida.format(date));
        } catch (Exception e) {
            ftxtFecha.setText(""); 
        }

        for (int i = 0; i < cmbCuentaBancaria.getItemCount(); i++) {
            if (cmbCuentaBancaria.getItemAt(i).toString().contains("Banco Agrícola")) { 
                cmbCuentaBancaria.setSelectedIndex(i);
                break;
            }
        }
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        if (idSeleccionado == 0) {
            JOptionPane.showMessageDialog(this, "Por favor, seleccione un cheque de la tabla.");
            return;
        }

        int confirmacion = JOptionPane.showConfirmDialog(this, "¿Seguro que desea eliminar este cheque?");
        if (confirmacion == 0) {
            ChequeDAO dao = new ChequeDAO();
            if (dao.eliminar(idSeleccionado)) {
                JOptionPane.showMessageDialog(this, "Cheque eliminado correctamente.");
                listar(); 
                idSeleccionado = 0; 
                limpiarCampos();
            }
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if (idSeleccionado == 0) {
            JOptionPane.showMessageDialog(this, "Por favor, seleccione un registro de la tabla primero.");
            return;
        }
    ...
    private void txtBuscarPorNombreKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtBuscarPorNombreKeyReleased
        String textoBusqueda = txtBuscarPorNombre.getText();
